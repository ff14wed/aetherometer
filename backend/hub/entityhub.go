...
// 	}
// }
// entityHub.Broadcast(models.EntityEventsPayload{StreamID: 1})
//
// Expected output:
// models.EntityEventsPayload{StreamID:1, EntityID:0, Type:models.EntityEventType(nil)}
type EntityHub struct {
	subscribers map[uint64]chan models.EntityEventsPayload
	baseSubID   uint64
	chanSize    int
...
func NewEntityHub(chanSize int) *EntityHub {
	return &EntityHub{
		subscribers: make(map[uint64]chan models.EntityEventsPayload),
		baseSubID:   0,
		chanSize:    chanSize,
	}
}
...
func (h *EntityHub) Broadcast(payload models.EntityEventsPayload) {
	subsList := []chan models.EntityEventsPayload{}
	h.subLock.Lock()
...
func (h *EntityHub) Subscribe() (chan models.EntityEventsPayload, uint64) {
	sub := make(chan models.EntityEventsPayload, h.chanSize)
